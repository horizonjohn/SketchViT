...
class Net_Basic(nn.Module):
    ...
    def Gumbel_Tree(self, V_Feature, Training=True):
        ...
        Correlation_Vector = Correlation_Matrix[
            torch.triu(torch.ones_like(Correlation_Matrix), diagonal=1) == 1].unsqueeze(0)
        Correlation_Indices = torch.triu(torch.ones_like(Correlation_Matrix), diagonal=1).nonzero()

        left_node = torch.repeat_interleave(V_Feature,
                                            torch.arange(start=num_nodes - 1, end=-1, step=-1, device='cuda'), dim=0)
        right_node = []
    ...
    def Train(self, sketch, sketch_bbox, positive, pos_bbox, negative, negative_bbox):
        ...
        loss = self.triplet(torch.stack(anchor_Batch[:positive.shape[0]]),
                            torch.stack(anchor_Batch[positive.shape[0]:]),
                            torch.stack(sample_batch[:positive.shape[0]]),
                            torch.stack(sample_batch[positive.shape[0]:]))
