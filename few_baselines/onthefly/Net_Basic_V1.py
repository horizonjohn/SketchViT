...
class Net_Basic(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.normalize(x, eps=1e-8)
        out = self.linear_out(out)  # [N, 64]
        embedding = F.normalize(out, eps=1e-8)
        return embedding
...
if __name__ == "__main__":
    model = Net_Basic()
    y = model(torch.randn(10, 3, 299, 299))   # torch.Size([10, 64])
    print(y.size())
    summary(model, (10, 3, 299, 299), depth=5)
